...
export function Form() {
  ...
  let [doorQuantity, setDoorQuantity] = useState(0);
  const [hasDoorType, setHasDoorType] = useState(false);

  const has_a_door_type = () => {
    if (doorType !== "" && doorType !== "Sem portas") {
      return true;
    } else {
      return false;
    }
  ...
  useEffect(() => {
    setHasDoorType(has_a_door_type);
      ...
      >
        ...
        <div className="flex flex-row">
          ...
          <label className="basis-1/3">
            ...
            <input
              type="number"
              // disabled={has_a_dwawer_type === true ? true : false}
